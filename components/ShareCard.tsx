          ...
          <div className="relative w-full h-full bg-[#09090b] rounded-[23px] overflow-hidden flex flex-col">
            ...
            <div className="flex justify-between items-center px-6 py-5 border-b border-white/5 relative z-10">
              <div className="flex items-center space-x-3">
                ...
                <span className={`text-white/60 text-[10px] tracking-[0.2em] uppercase ${jetbrains.className}`}>
                  GITHUB WRAPPED '26
                </span>
              </div>
              <div className={`text-green-400 text-[10px] tracking-[0.2em] font-bold uppercase ${jetbrains.className} flex items-center space-x-2`}>
            ...
            {/* Main Content */}
            <div className="p-8 pb-10 flex flex-col items-center relative z-10" style={{ transform: 'translateZ(30px)', transformStyle: 'preserve-3d' }}>
              ...
              <div className="flex flex-col items-center mb-10 transition-transform duration-200" style={{ transform: `translateX(${rotate.y * -0.5}px) translateY(${rotate.x * 0.5}px)` }}>
                ...
                <h1 className="text-3xl font-bold text-white tracking-tight">@{data.username}</h1>
                <div className="mt-3 px-3 py-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
                  <p className={`text-emerald-400 text-[10px] tracking-widest font-semibold ${jetbrains.className}`}>
                    ELITE CONTRIBUTOR
                  </p>
                </div>
              </div>
              ...
              <div className="w-full bg-gradient-to-b from-white/[0.08] to-transparent border border-white/10 rounded-2xl p-8 mb-4 flex flex-col items-center relative overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] transition-transform duration-200" style={{ transform: `translateX(${rotate.y * -0.3}px) translateY(${rotate.x * 0.3}px)` }}>
                <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-green-400/50 to-transparent"></div>
                <span className={`text-white/60 text-[11px] tracking-[0.2em] uppercase mb-3 ${jetbrains.className}`}>
                  Total Commits
                </span>
              ...
              <div className="grid grid-cols-2 gap-4 w-full">
                ...
                <StatCard 
                  ...
                  subValue={data.topLanguages.length > 0 ? `${data.topLanguages[0].percentage}%` : undefined} 
                  jetbrains={jetbrains.className} 
                />
                <StatCard 
                  label="Most Active" 
              ...
              <div className={`mt-12 text-[9px] text-white/30 tracking-[0.3em] uppercase ${jetbrains.className}`}>
                github-wrapped.com
              </div>
            </div>
          </div>
...
function StatCard({ label, value, subValue, jetbrains }: { label: string, value: string, subValue?: string, jetbrains: string }) {
  return (
    <div className="bg-white/[0.03] border border-white/5 hover:border-white/10 hover:bg-white/[0.05] transition-colors rounded-[16px] p-5 flex flex-col justify-between relative overflow-hidden group">
      ...
      <span className={`text-white/40 text-[9px] tracking-widest uppercase mb-4 ${jetbrains}`}>
        {label}
      </span>
      <div className="flex items-baseline space-x-2 relative z-10">
        <span className="text-xl font-bold text-white tracking-tight">{value}</span>
        {subValue && <span className={`text-emerald-400 text-xs font-semibold ${jetbrains}`}>{subValue}</span>}
      </div>
    </div>
  );
}
